...
% set detection root
det_root = '/home/rookie/github/iCAN/Results/HICO/1800000_iCAN_ResNet50_HICO/';
...
% set res file
res_root = '/home/rookie/github/iCAN/Results/HICO/1800000_iCAN_ResNet50_HICO/evaluation/result/%s/';
...
for i = 1:numel(list_coco_obj)
    ...
    hoi_int = find(ismember({list_action.nname}', list_coco_obj{i}));
    ...
    for i = 1:num_action
        ...
        switch format
            case 'obj'
                ...
                obj_id = find(ismember(list_coco_obj, nname));
                ...
                if size(ld.all_boxes, 1) > 0
                    idxs = find(cell2mat(ld.all_boxes(:, 4)) == (act_id - 1));
                    det = ld.all_boxes(idxs, :);
                    for ii = 1 : size(idxs, 1)
                        idx = find(ismember(anno.list_test, ['HICO_test2015_' num2str(ld.all_boxes{idxs(ii), 3}, '%08d') '.jpg']));
                        det{ii, 3} = idx;
                    end
                else
                    det = ld.all_boxes;
        ...
        for j = 1:size(det, 1)
            if ~isempty(det{j})
                ...
                det_id = [det_id; det{j, 3}];
                det_bb = [det_bb; [det{j, 1}, det{j, 2}]];
                det_conf = [det_conf; det{j, 5}];
            end
        end
        ...
        switch eval_mode
            case 'def'
                % do nothing
            case 'ko'
                nid = find(ismember({list_action.nname}', nname));  %#ok
